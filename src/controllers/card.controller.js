...
export const insertCard = async (req, res) => {
  ...
  if (!name || !type || !closingDay || !dueDate || !accountId) {
    return res
      .status(400)
...
export const deleteCard = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    // Trato erro de restrição de chave estrangeira caso o cartão possua transações
    if (error.code === "P2003") {
      return res.status(400).json({
        message:
          "Não é possível excluir o cartão pois ele possui transações vinculadas.",
      });
    }
...
export const inactiveCard = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erro ao alterar status do cartão." });
  }
};
