...
export const insertTransaction = async (req, res) => {
    ...
    const { type, value, date, description, categoryId, accountId, cardId, targetAccountId } = req.body;
    ...
    if (type === TransactionType.RECEITA) {
        if (!type || !value || !date || !description || !categoryId || !accountId) {
            return res.status(400).json({ message: "Para receitas, todos os campos e a conta são obrigatórios." });
        }
    ...
    if (type === TransactionType.DESPESA) {
        if (!type || !value || !date || !description || !categoryId || (!accountId && !cardId)) {
            return res.status(400).json({ message: "Para despesas, todos os campos e uma conta ou cartão são obrigatórios." });
        }
        if (accountId && cardId) {
            return res.status(400).json({ message: "A despesa não pode estar associada a uma conta e a um cartão simultaneamente." });
        }
        ...
        try {
            ...
            if (cardId) {
                ...
                const invoice = await prisma.invoice.upsert({
                    ...
                    update: { // Se a fatura já existe, apenas atualiza o valor
                        totalAmount: { increment: transactionValue },
                    },
                    create: { // Se não existe, cria uma nova
                        month, year,
    ...
    if (type === TransactionType.TRANSFERENCIA) {
        if (!value || !date || !accountId || !targetAccountId) {
            return res.status(400).json({ message: "Para transferências, valor, data, conta de origem e conta de destino são obrigatórios." });
        }
        ...
        try {
            ...
            console.error(error);
            return res.status(500).json({ message: "Erro ao realizar a transferência." });
        }
    }
    ...
    try {
        const updatedTransaction = await prisma.$transaction(async (prisma) => {
            ...
            if (originalTransaction.accountId) {
                const amountToRevert = originalTransaction.type === 'RECEITA' ? -originalValue : originalValue;
            ...
            if (accountId) {
                const amountToApply = originalTransaction.type === 'RECEITA' ? newValue : -newValue;
                await prisma.account.update({
                    where: { id: parseInt(accountId) },
        ...
        console.error(error);
        return res.status(500).json({ message: error.message || "Erro ao atualizar a transação." });
    }
};
    ...
    try {
        await prisma.$transaction(async (prisma) => {
            ...
            if (transaction.accountId) {
                ...
                const amountToRevert = transaction.type === 'DESPESA' ? valueToDelete : -valueToDelete;
                await prisma.account.update({
        ...
        console.error(error);
        return res.status(500).json({ message: error.message || "Erro ao excluir a transação." });
    }
};