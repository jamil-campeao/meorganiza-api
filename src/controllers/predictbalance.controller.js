...
export const generatePrediction = async (req, res) => {
  ...
  if (!n8nWebhookUrl) {
    return res.status(500).json({ message: "Webhook de previsão não configurado no servidor." });
  }
  ...
  try {
    ...
    if (!n8nResponse.ok) {
      ...
      throw new Error(errorBody.message || "Ocorreu um erro ao gerar a previsão.");
    }
